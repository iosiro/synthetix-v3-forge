...
contract RewardsManagerModuleTest is SynthetixLegacyBootstrapWithStakedPool {
    ...
    function testDistributeRewards() public {
        ...
        vm.expectRevert(
            abi.encodeWithSignature(
                "InvalidParameter(string,string)", "poolId-collateralType-distributor", "reward is not registered"
            )
        );
    ...
    function testInstantaneousRewardDistribution() public {
        ...
        rewardDistributor.distributeRewards(
            poolId, address(collateral), REWARD_AMOUNT, uint64(block.timestamp + 1 days), 0
        );
        ...
        uint256 availableRewards =
            synthetix.getAvailableRewards(accountId, poolId, address(collateral), address(rewardDistributor));
    ...
    function testOverTimeDistributionFromThePast() public {
        rewardDistributor.distributeRewards(
            poolId, address(collateral), REWARD_AMOUNT, uint64(block.timestamp - 100), 100
        );

        vm.warp(block.timestamp + 1);

        rewardDistributor.distributeRewards(
            poolId, address(collateral), REWARD_AMOUNT, uint64(block.timestamp - 50), 50
        );
    ...
    function testOverTimeDistributionFromInTheFuture() public {
        rewardDistributor.distributeRewards(
            poolId, address(collateral), REWARD_AMOUNT, uint64(block.timestamp + 200), 200
        );

        vm.warp(block.timestamp + 1);

        rewardDistributor.distributeRewards(
            poolId, address(collateral), REWARD_AMOUNT, uint64(block.timestamp - 50), 10
        );
    ...
    function testOverTimeDistributionDuring() public {
        ...
        // new distribution
        rewardDistributor.distributeRewards(
            poolId, address(collateral), REWARD_AMOUNT * 1000, uint64(block.timestamp - 110), 200
        );
        vm.warp(block.timestamp + 1);
    ...
    function testUpdateRewards() public {
        ...
        (uint256[] memory rewardAmounts, address[] memory distributorAddresses, uint256 poolRewardsCount) =
            synthetix.updateRewards(poolId, address(collateral), accountId);
    ...
    function testClaimRewards() public {
        ...
        vm.prank(user2);
        vm.expectRevert(
            abi.encodeWithSignature("PermissionDenied(uint128,bytes32,address)", accountId, bytes32("REWARDS"), user2)
        );
    ...
    function testRemoveRewardsDistributor() public {
        ...
        vm.expectRevert(
            abi.encodeWithSignature("InvalidParameter(string,string)", "distributor", "cant be re-registered")
        );
        synthetix.registerRewardsDistributor(poolId, address(collateral), address(rewardDistributor));
