...
contract VaultModuleTest is SynthetixLegacyBootstrapWithStakedPool {
    ...
    function test_vault_withdraw() public as_user(user1) {
        // deposit collateral
    ...
    function test_delegateCollateral() public {
        ...
        synthetix.delegateCollateral(accountId, poolId, address(collateral), delegateAmount, 1 ether);
    ...
    function test_whenCollateralIsDisabled() public {
        ...
        vm.prank(user1);
        vm.expectRevert(abi.encodeWithSignature("CollateralDepositDisabled(address)", address(collateral)));
        synthetix.delegateCollateral(accountId, fakePoolId, address(collateral), 25 ether, 1 ether);
