...
contract UtilsModuleTest is SynthetixLegacyTestBase {
    ...
    function testRegisterCcipSuccess() public {
        ...
        assertEq(addr, mockCcipAddress, "ccipChainlinkSend not set correctly");

        (addr,) = sUSD.getAssociatedSystem("ccipChainlinkRecv");

        assertEq(addr, mockCcipAddress, "ccipChainlinkRecv not set correctly");

        (addr,) = sUSD.getAssociatedSystem("ccipChainlinkTokenPool");

        assertEq(addr, mockCcipAddress, "ccipChainlinkTokenPool not set correctly");
    }
    ...
    function testConfigureOracleManagerSuccess() public {
        ...
        assertEq(synthetix.getOracleManager(), mockOracleManager, "Oracle manager not set correctly");
    }
    ...
    function testSetConfigSuccess() public {
        ...
        assertEq(synthetix.getConfig(key), value, "Config not set correctly");
    }
}
