...
contract CollateralModuleTest is SynthetixLegacyTestBase {   
    ...
    function test_withdrawTimeout() public {
...

        {
            ...
            (uint256 totalStaked, uint256 totalAssigned,) =
                synthetix.getAccountCollateral(1, address(collateral));
            uint256 totalAvailable = synthetix.getAccountAvailableCollateral(1, address(collateral));
    ...
    function test_createLockMulti() public as_user(user1) {
        ...
        CollateralLock.Data[] memory locks = ICollateralModule(address(synthetix)).getLocks(1, address(collateral), 0, type(uint256).max);
    ...
    function test_expiringLocks_1() public as_user(user1) {
        ...
        CollateralLock.Data[] memory locks = ICollateralModule(address(synthetix)).getLocks(1, address(collateral), 0, 0);
