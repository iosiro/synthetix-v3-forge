...
library OracleManagerForkTest {
    function upgrade(address ORACLE_MANAGER_PROXY) public {
        OracleManagerRouter oracleManagerRouter = new OracleManagerRouter(
            OracleManagerRouter.Modules({nodeModule: address(new NodeModule()), coreModule: address(new CoreModule())})
        );
